...
module.exports = {
    ...
    description: "subtract coins from a user's wallet",
    async execute(message, args, cmd, client, discord, profileData) {
        if(!message.member.roles.cache.has(process.env.ADMIN_ROLE)) return message.reply("You are not an admin");
        if(!args.length) return message.reply(`You need to mention a player to remove their coins`);
        const amount = args[1]
        ...
        try{
            ...
            return message.reply(`**$${amount} TC** subtracted from <@${target.id}> wallet`)
